    ...
    def GetNextMove(self, nextMove, GameStatus,yaml_file=None,weight=None):
        ...
        if self.mode=="train" or self.mode=="train_sample" or self.mode=="train_sample2":
            ...
            if self.double_dqn:
                ...
                next_backboard_hold  = self.getBoard(curr_backboard, next_shape_class, action[1], action[0])
                
                #画面ボードで テトリミノ回転状態 に落下させたときの次の状態一覧を作成
                next2_steps =self.get_next_func(next_backboard,next_piece_id,next_shape_class)
                next2_steps_hold =self.get_next_func(next_backboard_hold,next_piece_id,next_shape_class)
                # 次の状態一覧の action と states で配列化
                next2_actions, next2_states = zip(*next2_steps.items())
                next2_actions_hold, next2_states_hold = zip(*next2_steps_hold.items())
                # next_states のテンソルを連結
                # next2_states = torch.stack([next2_states, next2_states_hold], dim = 0)
                next2_states = torch.stack(next2_states)
                next2_states_hold = torch.stack(next2_states_hold)
                ...
                with torch.no_grad():
                    ...
                    next_predictions_hold = self.model(next2_states_hold)[:, 0]
                # 次の index を推論の最大値とする
                next_index = 0
                if torch.max(next_predictions).item() > torch.max(next_predictions_hold).item():
                    next_index = torch.argmax(next_predictions).item()
                else:
                    next_index = torch.argmax(next_predictions_hold).item()
                    isHold = True
                
                # next_index = torch.argmax(max(next_predictions, next_predictions_hold)).item()
                # 次の状態を index で指定し取得
